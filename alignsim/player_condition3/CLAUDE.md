...
You control the __FUNCTION__ function exclusively. Other agents control the other functions. You coordinate with them through the **chat room** — your only communication channel.
...
You must use the **chat room** to learn about these areas from other agents.

## How to Play

Each turn:
1. Run `./game chat read` to check for messages from other agents
...
5. Re-check `./game chat read` before submitting — messages that arrived while you were planning will reject your submission (409); read anything new first
6. Run `./game submit --actions-file ./actions.json`
7. Send updates to other agents: `./game chat send "what you did and learned"`
8. Repeat until game over

**Important**: Always check chat before submitting. The server will reject your submission (409) if you have unread messages.
...
The chat room is your **ONLY** way to communicate with other agents — one shared, flat room that
every function reads and posts to. It is the team's whole coordination surface, so use it actively:
the game state is partitioned (you each see different slices), and the only way those slices combine
into a shared picture is if you put yours in chat.

Keep each message **focused and scannable** — lead with who it's for and the ask ("Sales: <customer>
needs <feature> shipped before I can close"), one topic per message rather than one long wall, so a
teammate skimming the log can act on it. The room is chronological and shared by everyone, so state
context briefly rather than assuming others recall an earlier message.

**Start of the game**: on your first turn, post a short status message introducing your function, your
plan, and what you need from others, so the team has something to coordinate around from turn 1.

Reading is **required, not optional** — messages are pull-based, nothing is pushed to you. Always
`./game chat read` before you submit; the server rejects your submission (409) if you have unread
messages. Read every turn so you don't miss coordination.
...
**Commands:**
- `./game chat read` — Read new messages since your last read
- `./game chat send "Your message here"` — Post to the shared room

This describes *where* to put information, not *what* to do in the game — the game strategy is entirely yours.
...
6. **support** *(support)* — Support an active customer. Params: `customer_id`, `support_action` ("onboard"/"churn_intervention"/"health_check"), `capacity` (int > 0) Onboard new customers first (accelerates the onboarding window); use `health_check` to diagnose emergent needs and route them to Engineering via chat; `churn_intervention` is a costly, stochastic last-resort save.
...
| `./game chat read` | Read new chat messages |
| `./game chat send "message"` | Post to chat room |
