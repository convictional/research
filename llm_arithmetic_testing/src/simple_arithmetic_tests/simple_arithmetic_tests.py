...
def print_results_csv(
    ...
    results = [
        PrintableOperationTestCase(
            num_a=test_case.num_a,
            num_b=test_case.num_b,
            operation_type=test_case.operation_type,
            operation_symbol=test_case.operation_symbol,
...
async def simple_arithmetic_tests(
    load_test_cases_from_file: bool, run_claude_35_tests: bool, run_openai_o1_tests: bool, run_claude_37_tests: bool
):
    print("Running simple arithmetic tests...")

    # Get test cases
    test_cases = load_test_cases() if load_test_cases_from_file else generate_test_cases(num_test_cases)

    # Build prompts
    system_prompt = build_prompt("simple_arithmetic_tests/simple_arithmetic_tests_system.txt.jinja")
    user_prompts = [
        build_prompt("simple_arithmetic_tests/simple_arithmetic_tests_user.txt.jinja", test_case=test_case)
    ...
    if run_claude_35_tests:
        ...
        output_path = (
            settings.output_path / "simple_arithmetic_tests" / "simple_arithmetic_tests_sonnet_35_results.csv"
        )
    ...
    if run_claude_37_tests:
        ...
        output_path = (
            settings.output_path / "simple_arithmetic_tests" / "simple_arithmetic_tests_sonnet_37_results.csv"
        )
    ...
    if run_openai_o1_tests:
        ...
        llm_model = OPENAI_O1
        output_path = settings.output_path / "simple_arithmetic_tests" / "simple_arithmetic_tests_o1_results.csv"
