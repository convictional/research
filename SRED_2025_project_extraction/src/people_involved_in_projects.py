...
class ProjectSourceContent(BaseModel):
    content_data: list[SourceContent] = Field(..., title="List of source content data per project.")
...
def get_project_source_content_data(
    projects: list[HighLevelResearchAndDevelopmentProject], content_data: list[SourceContent]
...
async def extract_people_per_project_per_content_chunk(
    ...
    For each project, loop over the content data and extract the people involved for each chunk.
    ...
    for project, project_source_content in tqdm(
        ...
        user_prompts = [
            build_prompt(
                ...
                content_data_type=content.type,
                content_data=content.text_chunk,
            )
...
async def people_involved_in_projects(content_data: list[SourceContent]):
    """
