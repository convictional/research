...
async def group_projects_into_high_level_themes(
    ...
    print(f"LLM call took {llm_call_end_time - llm_call_start_time} seconds.")
...
async def get_summarized_projects_from_high_level_themes(
    ...
    # Sort summarized projects by the number of source content ids, descending
    summarized_projects.sort(key=lambda p: len(p.source_content_ids), reverse=True)
