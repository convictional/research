...
GOAL_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_4_goal_embeddings.pkl"
TASK_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_4_task_embeddings.pkl"
GOAL_KEYWORDS_AND_CATEGORIES_OUTPUT_PATH = settings.output_path / "approach_4_goal_keywords_and_categories.pkl"
TASK_KEYWORDS_AND_CATEGORIES_OUTPUT_PATH = settings.output_path / "approach_4_task_keywords_and_categories.pkl"
TASK_MATCH_RESULTS_CSV_OUTPUT_PATH = settings.output_path / "approach_4_results.csv"


class GoalKeywordsAndCategoriesResponse(BaseModel):
    keywords_and_categories: str = Field(..., title="The keywords and categories of the goal")


class TaskKeywordsAndCategoriesResponse(BaseModel):
    keywords_and_categories: str = Field(..., title="The keywords and categories of the task")
...
class MatchResult(BaseModel):
    ...
    goal_llm_keywords_and_categories: str = Field(
        ..., description="The LLM generated keywords and categories of the goal, used for the embedding"
    )
...
class TaskMatchResultsCSVRow(BaseModel):
    ...
    task_llm_keywords_and_categories: str = Field(
        ..., description="The LLM generated keywords and categories of the task, used for the embedding"
    )
...
async def approach_4(goals: list[Goal], tasks: list[Task], load_embeddings_from_cache: bool):
    """
    Approach 4 for matching platform tasks to goals.

    This approach uses embeddings of LLM generated keywords and categories of the tasks and goals to match them.
    """
    print("Starting matching platform tasks to goals using Approach 4...")
    ...
    if load_embeddings_from_cache:
        ...
        goal_keywords_and_categories: list[GoalKeywordsAndCategoriesResponse] = load_pickle_file(
            GOAL_KEYWORDS_AND_CATEGORIES_OUTPUT_PATH
        )
        task_keywords_and_categories: list[TaskKeywordsAndCategoriesResponse] = load_pickle_file(
            TASK_KEYWORDS_AND_CATEGORIES_OUTPUT_PATH
        )
    else:
        print("Loading embeddings using OpenAI API...")

        # Get goal keywords and categories from LLM
        print("Getting keywords and categories for goals...")

        system_prompt = build_prompt("approach_4_goal_keywords_and_categories_system.txt.jinja")
        user_prompts = [
            build_prompt("approach_4_goal_keywords_and_categories_user.txt.jinja", goal=goal) for goal in goals
        ]
        goal_keywords_and_categories: list[
            GoalKeywordsAndCategoriesResponse
        ] = await get_keywords_and_categories_from_llm(system_prompt, user_prompts, GoalKeywordsAndCategoriesResponse)

        # Get task keywords and categories from LLM
        print("Getting keywords and categories for tasks...")
        system_prompt = build_prompt("approach_4_task_keywords_and_categories_system.txt.jinja")
        user_prompts = [
            build_prompt("approach_4_task_keywords_and_categories_user.txt.jinja", task=task) for task in tasks
        ]
        task_keywords_and_categories: list[
            TaskKeywordsAndCategoriesResponse
        ] = await get_keywords_and_categories_from_llm(system_prompt, user_prompts, TaskKeywordsAndCategoriesResponse)

        # Save keywords and categories to cache
        print("Saving keywords and categories to cache...")
        ...
        goal_keywords_and_categories = [
            GoalKeywordsAndCategoriesResponse(
                keywords_and_categories=goal_keywords_and_categories.keywords_and_categories
            )
            for goal_keywords_and_categories in goal_keywords_and_categories
        ]
        task_keywords_and_categories = [
            TaskKeywordsAndCategoriesResponse(
                keywords_and_categories=task_keywords_and_categories.keywords_and_categories
            )
            for task_keywords_and_categories in task_keywords_and_categories
        ]
        dump_to_pickle_file(goal_keywords_and_categories, GOAL_KEYWORDS_AND_CATEGORIES_OUTPUT_PATH)
        dump_to_pickle_file(task_keywords_and_categories, TASK_KEYWORDS_AND_CATEGORIES_OUTPUT_PATH)

        # Get embeddings for goal keywords and categories
        print("Embedding goal keywords and categories...")
        goal_texts_to_embed = [
            goal_keywords_and_categories.keywords_and_categories
            for goal_keywords_and_categories in goal_keywords_and_categories
        ]
        goal_embeddings: list[list[float]] = await embed_list_of_texts(goal_texts_to_embed)

        # Get embeddings for task keywords and categories
        print("Embedding task keywords and categories...")
        task_texts_to_embed = [
            task_keywords_and_categories.keywords_and_categories
            for task_keywords_and_categories in task_keywords_and_categories
        ]
    ...
    print(f"Goal keywords and categories: {len(goal_keywords_and_categories)}")
    print(f"Task keywords and categories: {len(task_keywords_and_categories)}")
        ...
        for j, goal in enumerate(goals):
            task_match_results.append(
                MatchResult(
                    ...
                    goal_llm_keywords_and_categories=goal_keywords_and_categories[j].keywords_and_categories,
    ...
    for i, task in enumerate(tasks):
        tasks_csv_rows.append(
            TaskMatchResultsCSVRow(
                ...
                task_llm_keywords_and_categories=task_keywords_and_categories[i].keywords_and_categories,
                linked_goals="\n\n".join(
                    [
...
async def get_keywords_and_categories_from_llm(
    system_prompt: str,
    ...
    """
    Get keywords and categories for the given user prompts using the specified system prompt.
    """
    print("Getting keywords and categories...")
    ...
    keywords_and_categories: list[type[BaseModel]] = await execute_tasks_with_manual_pbar(tasks)
