...
GOAL_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_7_goal_embeddings.pkl"
TASK_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_7_task_embeddings.pkl"
GOAL_ENTITIES_OUTPUT_PATH = settings.output_path / "approach_7_goal_entities.pkl"
TASK_ENTITIES_OUTPUT_PATH = settings.output_path / "approach_7_task_entities.pkl"
TASK_MATCH_RESULTS_CSV_OUTPUT_PATH = settings.output_path / "approach_7_results.csv"
...
class MatchResult(BaseModel):
    ...
    goal_llm_entities: str = Field(..., description="The LLM generated entities of the goal, used for the embedding")
...
class TaskMatchResultsCSVRow(BaseModel):
    ...
    task_llm_entities: str = Field(..., description="The LLM generated entities of the task, used for the embedding")
...
async def approach_7(goals: list[Goal], tasks: list[Task], load_embeddings_from_cache: bool):
    """
    Approach 7 for matching platform tasks to goals.

    This approach uses embeddings of LLM generated entities of the tasks and goals to match them.
    """
    print("Starting matching platform tasks to goals using Approach 7...")
    ...
    if load_embeddings_from_cache:
        ...
        # Get goal entities from LLM
        print("Getting entities for goals...")

        system_prompt = build_prompt("approach_7_goal_entities_system.txt.jinja")
        user_prompts = [build_prompt("approach_7_goal_entities_user.txt.jinja", goal=goal) for goal in goals]
        goal_entities: list[GoalEntitiesResponse] = await get_entities_from_llm(
            system_prompt, user_prompts, GoalEntitiesResponse
        )

        # Get task entities from LLM
        print("Getting entities for tasks...")
        system_prompt = build_prompt("approach_7_task_entities_system.txt.jinja")
        user_prompts = [build_prompt("approach_7_task_entities_user.txt.jinja", task=task) for task in tasks]
        task_entities: list[TaskEntitiesResponse] = await get_entities_from_llm(
            system_prompt, user_prompts, TaskEntitiesResponse
        )
        ...
        for j, goal in enumerate(goals):
            task_match_results.append(
                MatchResult(
                    ...
                    goal_llm_entities=goal_entities[j].entities,
    ...
    for i, task in enumerate(tasks):
        tasks_csv_rows.append(
            TaskMatchResultsCSVRow(
                ...
                task_llm_entities=task_entities[i].entities,
                linked_goals="\n\n".join(
                    [
...
async def get_entities_from_llm(
    system_prompt: str,
    ...
    """
    Get entities for the given user prompts using the specified system prompt.
    """
