...
GOAL_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_5_goal_embeddings.pkl"
TASK_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_5_task_embeddings.pkl"
GOAL_KEYWORDS_OUTPUT_PATH = settings.output_path / "approach_5_goal_keywords.pkl"
TASK_KEYWORDS_OUTPUT_PATH = settings.output_path / "approach_5_task_keywords.pkl"
TASK_MATCH_RESULTS_CSV_OUTPUT_PATH = settings.output_path / "approach_5_results.csv"
...
class MatchResult(BaseModel):
    ...
    goal_llm_keywords: str = Field(..., description="The LLM generated keywords of the goal, used for the embedding")
...
class TaskMatchResultsCSVRow(BaseModel):
    ...
    task_llm_keywords: str = Field(..., description="The LLM generated keywords of the task, used for the embedding")
...
async def approach_5(goals: list[Goal], tasks: list[Task], load_embeddings_from_cache: bool):
    """
    Approach 5 for matching platform tasks to goals.

    This approach uses embeddings of LLM generated keywords of the tasks and goals to match them.
    """
    print("Starting matching platform tasks to goals using Approach 5...")
    ...
    if load_embeddings_from_cache:
        ...
        # Get goal keywords from LLM
        print("Getting keywords for goals...")

        system_prompt = build_prompt("approach_5_goal_keywords_system.txt.jinja")
        user_prompts = [build_prompt("approach_5_goal_keywords_user.txt.jinja", goal=goal) for goal in goals]
        goal_keywords: list[GoalKeywordsResponse] = await get_keywords_from_llm(
            system_prompt, user_prompts, GoalKeywordsResponse
        )

        # Get task keywords from LLM
        print("Getting keywords for tasks...")
        system_prompt = build_prompt("approach_5_task_keywords_system.txt.jinja")
        user_prompts = [build_prompt("approach_5_task_keywords_user.txt.jinja", task=task) for task in tasks]
        task_keywords: list[TaskKeywordsResponse] = await get_keywords_from_llm(
            system_prompt, user_prompts, TaskKeywordsResponse
        )
        ...
        for j, goal in enumerate(goals):
            task_match_results.append(
                MatchResult(
                    ...
                    goal_llm_keywords=goal_keywords[j].keywords,
    ...
    for i, task in enumerate(tasks):
        tasks_csv_rows.append(
            TaskMatchResultsCSVRow(
                ...
                task_llm_keywords=task_keywords[i].keywords,
                linked_goals="\n\n".join(
                    [
...
async def get_keywords_from_llm(
    system_prompt: str,
