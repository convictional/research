...
GOAL_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_6_goal_embeddings.pkl"
TASK_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_6_task_embeddings.pkl"
GOAL_CATEGORIES_OUTPUT_PATH = settings.output_path / "approach_6_goal_categories.pkl"
TASK_CATEGORIES_OUTPUT_PATH = settings.output_path / "approach_6_task_categories.pkl"
TASK_MATCH_RESULTS_CSV_OUTPUT_PATH = settings.output_path / "approach_6_results.csv"


class GoalCategoriesResponse(BaseModel):
    categories: str = Field(..., title="The categories of the goal")


class TaskCategoriesResponse(BaseModel):
    categories: str = Field(..., title="The categories of the task")
...
class MatchResult(BaseModel):
    ...
    goal_llm_categories: str = Field(
        ..., description="The LLM generated categories of the goal, used for the embedding"
    )
...
class TaskMatchResultsCSVRow(BaseModel):
    ...
    task_llm_categories: str = Field(
        ..., description="The LLM generated categories of the task, used for the embedding"
    )
...
async def approach_6(goals: list[Goal], tasks: list[Task], load_embeddings_from_cache: bool):
    """
    Approach 6 for matching platform tasks to goals.

    This approach uses embeddings of LLM generated categories of the tasks and goals to match them.
    """
    print("Starting matching platform tasks to goals using Approach 6...")
    ...
    if load_embeddings_from_cache:
        ...
        goal_categories: list[GoalCategoriesResponse] = load_pickle_file(GOAL_CATEGORIES_OUTPUT_PATH)
        task_categories: list[TaskCategoriesResponse] = load_pickle_file(TASK_CATEGORIES_OUTPUT_PATH)
    else:
        print("Loading embeddings using OpenAI API...")

        # Get goal categories from LLM
        print("Getting categories for goals...")

        system_prompt = build_prompt("approach_6_goal_categories_system.txt.jinja")
        user_prompts = [build_prompt("approach_6_goal_categories_user.txt.jinja", goal=goal) for goal in goals]
        goal_categories: list[GoalCategoriesResponse] = await get_categories_from_llm(
            system_prompt, user_prompts, GoalCategoriesResponse
        )

        # Get task categories from LLM
        print("Getting categories for tasks...")
        system_prompt = build_prompt("approach_6_task_categories_system.txt.jinja")
        user_prompts = [build_prompt("approach_6_task_categories_user.txt.jinja", task=task) for task in tasks]
        task_categories: list[TaskCategoriesResponse] = await get_categories_from_llm(
            system_prompt, user_prompts, TaskCategoriesResponse
        )
        print(task_categories[0])

        # Save categories to cache
        print("Saving categories to cache...")
        ...
        goal_categories = [GoalCategoriesResponse(categories=categories.categories) for categories in goal_categories]
        task_categories = [TaskCategoriesResponse(categories=categories.categories) for categories in task_categories]
        dump_to_pickle_file(goal_categories, GOAL_CATEGORIES_OUTPUT_PATH)
        dump_to_pickle_file(task_categories, TASK_CATEGORIES_OUTPUT_PATH)

        # Get embeddings for goal categories
        print("Embedding goal categories...")
        goal_texts_to_embed = [categories.categories for categories in goal_categories]
        print(goal_texts_to_embed[0])
        goal_embeddings: list[list[float]] = await embed_list_of_texts(goal_texts_to_embed)

        # Get embeddings for task categories
        print("Embedding task categories...")
        task_texts_to_embed = [categories.categories for categories in task_categories]
    ...
    print(f"Goal categories: {len(goal_categories)}")
    print(f"Task categories: {len(task_categories)}")
        ...
        for j, goal in enumerate(goals):
            task_match_results.append(
                MatchResult(
                    ...
                    goal_llm_categories=goal_categories[j].categories,
    ...
    for i, task in enumerate(tasks):
        tasks_csv_rows.append(
            TaskMatchResultsCSVRow(
                ...
                task_llm_categories=task_categories[i].categories,
                linked_goals="\n\n".join(
                    [
...
async def get_categories_from_llm(
    system_prompt: str,
    ...
    """
    Get categories for the given user prompts using the specified system prompt.
    """
