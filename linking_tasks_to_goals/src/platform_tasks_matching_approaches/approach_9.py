...
GOAL_KEYWORDS_OUTPUT_PATH = settings.output_path / "approach_9_goal_keywords.pkl"
TASK_KEYWORDS_OUTPUT_PATH = settings.output_path / "approach_9_task_keywords.pkl"
GOAL_ENTITIES_OUTPUT_PATH = settings.output_path / "approach_9_goal_entities.pkl"
TASK_ENTITIES_OUTPUT_PATH = settings.output_path / "approach_9_task_entities.pkl"
GOAL_KEYWORDS_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_9_goal_keywords_embeddings.pkl"
TASK_KEYWORDS_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_9_task_keywords_embeddings.pkl"
GOAL_ENTITIES_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_9_goal_entities_embeddings.pkl"
TASK_ENTITIES_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_9_task_entities_embeddings.pkl"
TASK_MATCH_RESULTS_CSV_OUTPUT_PATH = settings.output_path / "approach_9_results.csv"
...
async def approach_9(goals: list[Goal], tasks: list[Task], load_embeddings_from_cache: bool):
    """
    Approach 9 for matching platform tasks to goals.
    ...
    This approach uses a multi-step process to match tasks to goals.
        ...
        - 6. Give the remaining goals for each task to an LLM to judge the final match.
             The LLM is explicitly told to think about assumptions it is making and to only use
             assumptions with a High confidence level.

    This approach is designed to be more robust and accurate than previous approaches,
    and is heavily based on Approach 8.
    """
    print("Starting matching platform tasks to goals using Approach 9...")
...
async def get_keywords(
    ...
    system_prompt = build_prompt("approach_9_goal_keywords_system.txt.jinja")
    user_prompts = [build_prompt("approach_9_goal_keywords_user.txt.jinja", goal=goal) for goal in goals]
    ...
    system_prompt = build_prompt("approach_9_task_keywords_system.txt.jinja")
    user_prompts = [build_prompt("approach_9_task_keywords_user.txt.jinja", task=task) for task in tasks]
...
async def get_entities(
    ...
    system_prompt = build_prompt("approach_9_goal_entities_system.txt.jinja")
    user_prompts = [build_prompt("approach_9_goal_entities_user.txt.jinja", goal=goal) for goal in goals]
    ...
    system_prompt = build_prompt("approach_9_task_entities_system.txt.jinja")
    user_prompts = [build_prompt("approach_9_task_entities_user.txt.jinja", task=task) for task in tasks]
...
async def do_llm_as_a_judge_evaluations(augmented_tasks: list[AugmentedTask]) -> list[AugmentedTask]:
    ...
    for i, task in enumerate(augmented_tasks):
        ...
        system_prompt = build_prompt("approach_9_llmaaj_system.txt.jinja")
        user_prompts = [
            build_prompt("approach_9_llmaaj_user.txt.jinja", task=task, goal=goal) for goal in embedding_filtered_goals
        ]
