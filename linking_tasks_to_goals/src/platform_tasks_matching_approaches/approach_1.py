...
GOAL_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_1_goal_embeddings.pkl"
TASK_EMBEDDINGS_OUTPUT_PATH = settings.output_path / "approach_1_task_embeddings.pkl"
TASK_MATCH_RESULTS_CSV_OUTPUT_PATH = settings.output_path / "approach_1_results.csv"
...
async def approach_1(goals: list[Goal], tasks: list[Task], load_embeddings_from_cache: bool):
    """
    Approach 1 for matching platform tasks to goals.

    In this approach we use embeddings to match tasks to goals.
        - Task embeddings: title + description
        - Goal embeddings: title + all success condition descriptions
    """
    print("Starting matching platform tasks to goals using Approach 1...")
    ...
    if load_embeddings_from_cache:
        ...
        goal_texts_to_embed = [
            f"{goal.title} {' '.join([sc.description for sc in goal.success_conditions])}" for goal in goals
        ]
        goal_embeddings: list[list[float]] = await embed_list_of_texts(goal_texts_to_embed)

        # Embed tasks
        print("Embedding tasks...")
        task_texts_to_embed = [f"{task.title} {task.description}" for task in tasks]
