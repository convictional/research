...
DECISON_ANALYSIS_UNWRITTEN_RULES_RESULTS_PATH = OUTPUT_PATH_ROOT / "decision_analysis_unwritten_rules_results.csv"


async def process_decision_analysis_unwritten_rules_requests(
    ...
    That is:
    - Filter requests for decision analysis unwritten rules
    ...
    """
    print("Processing decision analysis unwritten rules requests...")

    # Filter requests for decision analysis unwritten rules
    print("Filtering requests for decision analysis unwritten rules...")
    decision_analysis_unwritten_rules_df = request_data_df[request_data_df["is_decision_analysis_unwritten_rules"]]
    print(f"Number of decision analysis unwritten rules requests: {len(decision_analysis_unwritten_rules_df)}")

    parsed_request_bodies_without_goals: list[dict] = get_request_bodies_and_remove_goals_from_system_prompt(
        decision_analysis_unwritten_rules_df
    )
    ...
    for (_, row), request_body_without_goals, response_without_goals in zip(
        decision_analysis_unwritten_rules_df.iterrows(), parsed_request_bodies_without_goals, responses_without_goals
    ):
    ...
    dump_list_of_objects_to_csv(response_comparisons, DECISON_ANALYSIS_UNWRITTEN_RULES_RESULTS_PATH)
