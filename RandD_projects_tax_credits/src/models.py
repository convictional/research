...
class GitHubSourceContent(BaseModel):
    content_id: str = Field(..., title="Content ID, created by concatenating repository_full_name and issue_number")
    title: str = Field(..., title="Issue title")
    content: str = Field(..., title="Original unchunked content of the source data")
    created_at: datetime = Field(..., title="When the issue was created")
    last_comment_at: datetime = Field(..., title="When the last comment was made")
    closed_at: datetime | None = Field(..., title="When the issue was closed")
    chunk_index: int = Field(..., title="Index of the text chunk")
    text_chunk: str = Field(
        ...,
        title="Text chunk of the content. This is the content split into chunks. Each text chunk has the issue title and the text chunk content.",
    )
    url: str = Field(..., title="URL to the issue")
