...
def load_and_randomize_projects(input_file_path: Path) -> list[ResearchAndDevelopmentProject]:
...
async def summarize_projects_iteration(
    ...
    projects: list[ResearchAndDevelopmentProject] = load_and_randomize_projects(input_file_path)
...
async def fine_grained_projects_into_coarse_grained_projects():
    ...
    Note, if max_size_of_project_groups is too large and the there aren't many projects to group together,
    ...
    # iteration 4 # 69 projects result
    await summarize_projects_iteration(
